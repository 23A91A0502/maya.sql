...
/
--another pl/sql block to call the function

declare
     res_ int;
begin
    res_:=get_dept_salary('HR');
    dbms_output.put_line('salary is: ' || res_);
end;
/

--procedures (void functions)
-- stored procedures
--procedures doesn't return any value
-- Procedures primary used to minimize the time we spend
--to perform delete and update and insert operations
--on table

--procedures are of two types
--1.procedures with parameters
--2.procedures without parameters
--There are three different types of procedures we can use
-- IN parameters (inputs)
-- OUT parameters (output)
-- IN OUT parameter ( Can be both input and output)
--syntax of creating a procedure
--create or replace procedure pro_name[(with parameters only)] is|as
...
--1.using exec keyword
exec say_hello();
--2.calling the procedure directly in another pl/sql block
begin
   say_hello();
end;
/
-- procedure that prints numbers from 1 to N.
create or replace procedure get_num(n int) is
begin
    for i in 1..n loop
    dbms_output.put(i || ' ');
...
-- • Writeandcall aPL/SQLprocedurethat takes emp_idandnew_salary(UseINparamters)
--and UPDATEs the salary of the employee with given id to new_salary
create or replace procedure upd_sal(emp_id int ,new_sal int)
is
begin
     update employee
     set salary=new_sal
...
exec upd_sal(21,87000);
--Write and call a PL/SQL procedure that takes dept_name and new_value (integer) (Use
--IN paramters) and UPDATEs the ages of all the employees who belong the department given,
--to new_value where age is NULL

create or replace procedure upd_age(dept varchar,new_val int)
is
begin
    update employee
    set age=new_val
    where age is null and department = dept;
end;
/
exec upd_age('Operations',34);
select * from employee
where department='Operations';

--• Write and call a PL/SQL procedure that takes dept_name and DELETEs all the records
-- of that department where email is missing
create or replace procedure del_email(dept varchar) is
begin
    delete from employee
    where department=dept and email is null;
end;
/
exec del_email('SW');
select * from employee 
...
-- • Write and call a PL/SQL procedure that takes doj, min_salary and UPDATEs the salary
-- by 10% of all the employees whose date 
--of joining is greater than doj and salary is less than
-- min_salary
